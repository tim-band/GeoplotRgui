#! /bin/sh
set -eu
scfile=/usr/local/sbin/geoplotrctl
nxfile=/etc/nginx/conf.d/geoplotr.conf
max_processes=99
if [ -z "${1-}" ]
then
echo 'Usage:'
echo 'configureGeoplotR.sh 6'
echo 'sets GeoplotR to use 6 processes'
echo 'configureGeoplotR.sh 5 3901'
echo 'sets GeoplotR to use 5 processes on port numbers 3901 to 3906'
exit
